...
Rafa      | Improve Desing
MHD444    | Added Mobile Support

]]

local Release = "Galaxy Edition"
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if not isfolder("Galaxy Hub") and not isfile("Galaxy Hub/Startup Note") then
        delay(4, function()
            qNotePrompt({Title = 'Galaxy Hub Team', Description = 'We Are Not Responsible For Any Harm Caused By The Script, Use At Your Own Risk! (Our Scripts Are 99% Safe)',})
        end)
        makefolder("Galaxy Hub")
        writefile("Galaxy Hub/Note")
    end
...
-- Roblox Mobile - UWP  Hide / Unhide Button

if game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("ArrayfieldScreen") then
	game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("ArrayfieldScreen"):Destroy()
end

local ArrayfieldScreen   = Instance.new("ScreenGui")
local ArrayfieldToggleUI = Instance.new("TextButton")
local ArrayfieldCornerUI = Instance.new("UICorner")
local ArrayfieldImageUI  = Instance.new("ImageLabel")

ArrayfieldScreen.Name = "ArrayfieldScreen"
ArrayfieldScreen.Parent = game.CoreGui
ArrayfieldScreen.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

ArrayfieldToggleUI.Name = "ArrayfieldScreen"
ArrayfieldToggleUI.Parent = ArrayfieldScreen
ArrayfieldToggleUI.BackgroundColor3 = Color3.fromRGB(31,31,31)
ArrayfieldToggleUI.BorderSizePixel = 0
ArrayfieldToggleUI.Position = UDim2.new(0.120833337, 0, 0.0952890813, 0)
ArrayfieldToggleUI.Size = UDim2.new(0, 50, 0, 50)
ArrayfieldToggleUI.Font = Enum.Font.SourceSans
ArrayfieldToggleUI.Text = ""
ArrayfieldToggleUI.TextColor3 = Color3.fromRGB(0, 0, 0)
ArrayfieldToggleUI.TextSize = 14.000
ArrayfieldToggleUI.Draggable = true
ArrayfieldToggleUI.MouseButton1Click:Connect(function()
    if Debounce then return end
    if Hidden then
        Hidden = false
        Unhide()
...
ArrayfieldCornerUI.Name = "ArrayfieldCornerUI"
ArrayfieldCornerUI.Parent = ArrayfieldToggleUI

ArrayfieldImageUI.Name = "GalaxyHubImage"
ArrayfieldImageUI.Parent = ArrayfieldToggleUI
ArrayfieldImageUI.BackgroundColor3 = Color3.fromRGB(192,192,192)
ArrayfieldImageUI.BackgroundTransparency = 1.000
	...
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				...
				RayfieldLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for the script has been loaded from a previous session"})
			end
		end)
	end
end
